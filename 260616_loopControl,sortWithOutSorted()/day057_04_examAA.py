...
i, j, num = 0, 0, 1;
while num:
    ln = input('3 이상의 홀수의 줄수를 입력하세요 : ')
    ln = (int(ln) if ln.isdigit() else 0)                 ### 변경부분
#    print(type(ln))
#    print('ln-->',ln)
#    print('num-->',num)
                                                         ### 변경부분
    if ln % 2 == 0 or ln < 3 or not(str(ln).isdigit()):   # 3 이상의 짝수가 아니면 다시입력하도록 함
        print('입력한 수가 틀렸습니다.')                  
        continue
    ...
    while True:
        ...
        stopNgo = input('0.종료 1.계속 : ')
        test = (int(stopNgo) if stopNgo.isdigit() else stopNgo)  ### 변경부분
        if test == 0 :
            print('프로그램 종료')
            exit(0)
        elif test == 1 :
