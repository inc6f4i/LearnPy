# 260625-실습-2-해답-ver1 (단톡방버전 : print 추가)
# 클래스 정의
class calc_class :
    # 변수 선언
    x = 0
    y = 1
    # x = y = 0
    print('0-1--->',id(x))  #####
    print('0-2--->',id(y))  #####
    # 생성자 : 객체 생성 + [멤버변수 초기화]
    def __init__(self, a, b):
        print('~~객체 생성~~')
        self.x = a # 10
        self.y = b # 20
        print('1-1--->',id(self.x)) #####
        print('1-2--->',id(self.y)) #####
    # 멤버 함수(기능)
    def plus(self): # self : 멤버(변수+함수) 참조 객체
        p = self.x + self.y
        print('2-1--->',id(self.x)) #####
        print('2-2--->',id(self.y)) #####
        print('2-3--->',id(p))      #####
        '''
        p : 지역변수
        self.x, self.y : 전역변수  
        '''
        return p
    def minus(self):
        m = self.x - self.y
        print('2-4--->',id(m))      #####
        return m
# class(1) -> object(n) 생성
obj1 = calc_class(10, 20) # 생성자 -> 객체1  
##
print('32line--->',id(calc_class.x), calc_class.x)  #####
print('33line--->',id(calc_class.y), calc_class.y)  #####
##
# object.member()  
#print('plus = ', obj1.plus()) # plus =  30
#print('minus =', obj1.minus()) # minus = -10
obj1.plus()
obj1.minus()
#
obj03 = calc_class(70,90)
#print('plus = ', obj03.plus())
#print('minus =', obj03.minus())
obj03.plus()
obj03.minus()


'''
        p : 지역변수
        self.x, self.y : 객체변수(함수 공부할 때 전역변수와 비슷한 성격)  '''
