...
if os.path.exists(counter_file):
    ...
    if len(lines) >= 1 and lines[1].strip():# 두번째 줄 보내는학생이름
        student_name = lines[1].strip()

    if len(lines) >= 1:#세번째 줄 CC 참조주소
        cc_email = lines[2].strip()

    if len(lines) >= 1 and "," in lines[3]: #네번째 줄 날짜기반 카운터, 자동갱신되나 오류시 이부분 텍스트파일수정
...
print(f"workSpaceOnWithChrome -> {subject} / 이름: {student_name} / CC: {cc_email or '없음'}")